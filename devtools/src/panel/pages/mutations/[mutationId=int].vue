...
import {
  useDuplexChannel,
  useMutationEntries,
} from "../../composables/duplex-channel";
...
const lastUpdate = useTimeAgo(
  () => selectedMutation.value?.devtools.updatedAt ?? 0,
  {
    ...TIME_AGO_OPTIONS,
    updateInterval: 3000,
  },
...
const isDataOpen = useLocalStorage<boolean>(
  "pc:mutation:details:data:open",
  false,
  {},
);
const isVarsOpen = useLocalStorage<boolean>(
  "pc:mutation:details:vars:open",
  true,
  {},
);
let wasDataOpen = isDataOpen.value;
let lastStatus: DataStateStatus | null = null;
const isErrorOpen = useLocalStorage<boolean>(
  "pc:mutation:details:error:open",
  false,
  {},
);
  ...
  >
    <template v-if="selectedMutation">
      <UCollapse title="Details" :icon="IInfoCircle">
        <div class="py-1 text-sm">
          <p class="grid grid-cols-[auto_1fr] gap-1">
            <span>id:</span>
            <span>
              <code class="rounded bg-neutral-500/20 p-0.5">{{
                selectedMutation.id
              }}</code>
            </span>
          </p>
          ...
          >
            ...
            >
              ...
              <span class="font-bold">{{
                formatTimeAgo(
                  new Date(
                    selectedMutation.devtools.inactiveAt +
                      selectedMutation.options.gcTime,
                  ),
                  {
                    ...TIME_AGO_OPTIONS,
                    max: undefined,
      ...
      <UCollapse title="Actions" :icon="IWrench">
        <div class="py-2 gap-2 flex flex-wrap items-center justify-items-start">
          <UButton
            ...
            @click="
              channel.emit('mutations:simulate:loading', selectedMutation.id)
            "
          >
            <i-lucide-loader />
            Simulate loading
          ...
          <UButton
            ...
            @click="
              channel.emit('mutations:simulate:error', selectedMutation.id)
            "
          >
          ...
          <UButton
            ...
            @click="
              channel.emit('mutations:simulate:error:stop', selectedMutation.id)
            "
          >
      ...
      <UCollapse
        ...
        :class="[
          selectedMutation.vars === undefined && 'text-(--ui-text-muted)',
        ]"
        :icon="IVariable"
        no-padding
      >
      ...
      <UCollapse
        ...
        :icon="IFileText"
        :class="[
          selectedMutation.state.data === undefined && 'text-(--ui-text-muted)',
        ]"
        no-padding
      >
