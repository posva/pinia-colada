...
```
[PC_R0002] The query cache cannot be directly set, it must be modified instead. This will fail in production.
├▶ fix: Use the cache methods (e.g. "setQueryData()", "setEntryState()") to modify the cache instead of replacing it.
╰▶ see: https://pinia-colada.esm.dev/errors/pc_r0002.html
```
