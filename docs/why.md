...
- [**vue-concurrency**](https://vue-concurrency.netlify.app/): vue-concurrency handles any async operation via `Task` and `TaskInstance` abstractions with full reactive async state tracking. It offers automatic task cancellation on component unmount as well as easy concurrency management -- tasks can be chained for co-dependency, run in parallel, set as restartable, cancelled, or enqueued to be performed later.
