...
- [**vue-concurrency**](https://vue-concurrency.netlify.app/): vue-concurrency too handles any async operation via `Task` and `TaskInstance` abstractions and full reactive async state tracking. Offers automatic task cancellation on component dismounts as well as easy concurrency management -- Tasks easily be chained for co-depencency, can be run in parallel, set as restartable, cancelled or enqueued to be performed later.
